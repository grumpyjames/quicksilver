...
quantity :: Order -> Int
quantity (Order _ q) = q

totalLiquidity :: Orderbook -> Int
totalLiquidity (os, ps) = sum (map quantity (os ++ ps))
...
runTests = hspec $ do
  describe "Orderbooks" $ do
    ...
    it "has the same liquidity sum regardless of the entry order of n orders" $ property $ \os ->
      True ==> (totalLiquidity (placeInstructions os)) == (totalLiquidity (placeInstructions $ reverse os))